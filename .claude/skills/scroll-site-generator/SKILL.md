...
`scripts/hf.py` (image | upload | video) reads HF_KEY (`key:secret`) from the
environment or from a `.env` sitting next to this SKILL.md. That `.env` is
gitignored — copy `.env.example` to `.env` and fill in the Higgsfield key
before the first generation. Platform facts that will save you an hour:
